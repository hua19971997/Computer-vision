...
vector<int> match_descriptors_a2b(const vector<Descriptor>& a, const vector<Descriptor>& b)
  ...
  for(int j=0;j<(int)a.size();j++)
    ...
    float cur = best_distance;
    for (int i = 0; i < (int)b.size(); i++) {
        cur = l1_distance(a[j].data, b[i].data);
...
vector<Match> match_descriptors(const vector<Descriptor>& a, const vector<Descriptor>& b)
  ...
  for (int i = 0; i < n; i++) {
      if (i == b2a[a2b[i]]) {
          float distance = l1_distance(a[i].data, b[a2b[i]].data);

          const Descriptor* aa = &a[i];
          const Descriptor* bb = &b[a2b[i]];
          Match best{ aa,bb,distance };
          m.push_back(best);
      }
  }
  return m;
...
Point project_point(const Matrix& H, const Point& p)
  ...
  // Have to divide by.... something...
    ...
    mp(2, 0) = 1;
    Matrix res = H * mp;
    float x = res(0, 0) / res(2, 0);
    float y = res(1, 0) / res(2, 0);
...
Matrix compute_homography_ba(const vector<Match>& matches)
  ...
  for(int i = 0; i < (int)matches.size(); ++i)
    ...
    vector<vector<double>> M1{ {mx, my, 1, 0, 0, 0, -nx * mx, -nx * my},
                                 {0, 0, 0, mx, my, 1, -ny * mx, -ny * my} };
    for (int j = 0; j < 2; j++) {
        for (int k = 0; k < 8; k++) {
            // fill matrix M
            M(i * 2 + j, k) = M1[j][k];
...
Matrix RANSAC(vector<Match> m, float thresh, int k, int cutoff)
  ...
  for (int i = 0; i < k; i++) {
      randomize_matches(m);
      vector<Match> new_m(m.begin(), m.begin() + 4);
      Matrix H = compute_homography_ba(new_m);
      vector<Match> inliers = model_inliers(H, m, thresh);
