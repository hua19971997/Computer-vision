...
void do_helens(void)
  ...
  th.push_back(thread([&](){create_panorama(im,"0-1","1","0" ,2,0,0.05,11,7,5,150000,100,0.5);}));
  th.push_back(thread([&](){create_panorama(im,"2-3","2","3" ,2,0,0.05,11,7,5,150000,100,0.5);}));
  th.push_back(thread([&](){create_panorama(im,"4-5","4","5" ,2,0,0.05,15,5,5,50000,100,0.5);}));
  for(auto&e1:th)e1.join();th.clear();
  
  
  create_panorama(im,"0--3", "2-3", "0-1",2,0,0.05,11,7,5,50000,100,0.5);
  create_panorama(im,"all" , "4-5", "0--3", 2,0,0.05,11,7,5,50000,100,0.5);
