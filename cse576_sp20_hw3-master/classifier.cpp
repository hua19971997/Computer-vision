...
Matrix backward_xw(const Layer &l, const Matrix &grad_y) {
  ...
  //NOT_IMPLEMENTED();
  grad_xw = backward_activate_matrix(l.out2, grad_y, l.activation);
  return grad_xw;
}
...
void update_layer(Layer &l, double rate, double momentum, double decay) {
  ...
  // NOT_IMPLEMENTED();
  Matrix delta_w_t = l.grad_w - decay * l.w + momentum * l.v;
  l.v = delta_w_t;

  // TODO: update the weights and save to l.w.
  // Hint: w_{t+1} = w_t + ηΔw_t
  l.w = l.w + rate * delta_w_t;
  //NOT_IMPLEMENTED();
}
...
Matrix Model::loss_derivative(const Matrix &y, const Matrix &p)
  {
    ...
    {
    Matrix d=y*0;
    for(int q1=0;q1<d.rows;q1++){
        for(int q2=0;q2<d.cols;q2++) {
            if(abs(y(q1, q2) - p(q1, q2)) > 1)
                d(q1, q2) = 2 * (y(q1, q2) > p(q1, q2)) - 1;
            else
                d(q1, q2) = 2 * (y(q1, q2) - p(q1, q2));
        }
    }
    return d;
    }
